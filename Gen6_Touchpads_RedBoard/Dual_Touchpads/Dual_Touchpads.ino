...
void printKeyboardReportEvents(uint8_t i2c_channel, HID_report_t * cur_report, HID_report_t * prev_report)
{
    ...
    {
        Serial.printf("I2C_Chan %d ->   Modifier change: 0x%02X (was 0x%02X)\n", 
            i2c_channel, cur_report->keyboard.modifier1, prev_report->keyboard.modifier1);
    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (cur_report->keyboard.keycode[i] != 0)
        {
            ...
            if (!foundInPrevious)
            {
                Serial.printf("I2C_Chan %d ->   Key pressed: 0x%02X\n", i2c_channel, cur_report->keyboard.keycode[i]);
                keyChanged = true;
            }
        }
    }
    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (prev_report->keyboard.keycode[i] != 0)
        {
            ...
            if (!foundInCurrent)
            {
                Serial.printf("I2C_Chan %d ->   Key released: 0x%02X\n", i2c_channel, prev_report->keyboard.keycode[i]);
                keyChanged = true;
            }
        }
    }
