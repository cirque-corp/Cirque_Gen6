...
uint16_t prevPtpX1, prevPtpY1;
uint16_t prevPtpX2, prevPtpY2;
bool prevPtpSetFlag1 = false;
bool prevPtpSetFlag2 = false;
...
void setup()
{
  ...
  prevButton1Pressed = false;
  prevButton2Pressed = false;
}
...
void printDecodeFailureDebug(uint8_t i2c_channel, const uint8_t* packet, uint8_t dr_status)
{
  ...
  {
    Serial.printf("I2C_Chan %d -> Possible cause: unexpected report ID (not PTP/MOUSE/KEYBOARD)\n", i2c_channel);
  }
...
void printEvent(uint8_t i2c_channel, HID_report_t* cur_report)
{
  switch( cur_report->reportID)
  {
    ...
    case PTP_REPORT_ID:
        ...
        break;
    case KEY_REPORT_ID:
        printKeyboardReportEvents(i2c_channel, cur_report, &prevKeyboardReport_g);
...
void printKeyboardReportEvents(uint8_t i2c_channel, HID_report_t * cur_report, HID_report_t * prev_report)
{
    ...
    {
        Serial.printf("I2C_Chan %d ->   Modifier change: ", i2c_channel);
    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (cur_report->keyboard.keycode[i] != 0)
        {
            ...
            if (!foundInPrevious)
            {
                Serial.printf("I2C_Chan %d ->   Key pressed: ", i2c_channel);
    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (prev_report->keyboard.keycode[i] != 0)
        {
            ...
            if (!foundInCurrent)
            {
                Serial.printf("I2C_Chan %d ->   Key released: ", i2c_channel);
